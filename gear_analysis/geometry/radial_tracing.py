...
from gear_analysis.utils import (
    normalize_angle, normalize_angle_diff, compute_angles, 
    compute_radii, fit_line_svd, unit_vector, orient_direction_outward
)
...
class RadialFlankTracer:
    ...
    def find_tooth_tips(
        ...
        angles = normalize_angle(compute_angles(tip_candidates))
        ...
        for i in range(n_teeth):
            ...
            if len(sector_points) > 0:
                ...
                sector_radii = compute_radii(sector_points)
    ...
    def trace_flank(
        ...
        current_angle = compute_angles(current_point[None, :])[0]
        ...
        while current_radius > r_inner and iteration < max_iterations:
            ...
            if len(flank_points) >= 2:
                ...
                flank_direction = unit_vector(next_point - flank_points[0])
        ...
        fitted_direction = orient_direction_outward(fitted_direction, centroid)
    ...
    def _find_next_point(
        ...
        angle_diff = normalize_angle_diff(angles - current_angle)
    ...
    def _fit_line(self, points: Points2D) -> Tuple[Vector2D, Vector2D, float]:
        ...
        """
        if len(points) < 2:
            centroid = points.mean(axis=0) if len(points) > 0 else np.array([0.0, 0.0])
            return centroid, np.array([1.0, 0.0]), 0.0
        
        centroid, direction = fit_line_svd(points)
        
        # Compute linearity score: ratio of singular values
